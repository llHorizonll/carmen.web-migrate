...
import { IconCalendar, IconHash, IconUserCircle, IconFileText, IconUser, IconClock, IconCoin, IconPercentage } from '@tabler/icons-react';
import type { ArInvoice, ArInvoiceDetail } from '../types';

interface DialogViewARInvoiceProps {
  opened: boolean;
  onClose: () => void;
  data: ArInvoice | null;
}
...
export function DialogViewARInvoice({ opened, onClose, data }: DialogViewARInvoiceProps) {
  if (!data) return null;

  const subTotal = data.Detail?.reduce((sum, d) => sum + (d.AmountBase || 0), 0) || 0;

  return (
    <Modal opened={opened} onClose={onClose} title="AR Invoice Detail" size="xl">
      <Stack gap="md">
        {/* Header Information */}
        <Paper withBorder p="md">
          ...
          <Grid gutter="md">
            <Grid.Col span={{ base: 12, md: 6 }}>
              ...
              <Text fw={500}>{data.ProfileCode} - {data.ProfileName}</Text>
            </Grid.Col>
            <Grid.Col span={{ base: 12, md: 6 }}>
              <Group gap="xs">
        ...
        <Paper withBorder p="md">
          ...
          <ScrollArea>
            <Table striped highlightOnHover>
              ...
              <Table.Tbody>
                {data.Detail?.map((line: ArInvoiceDetail, index: number) => (
                  <Table.Tr key={line.ArInvdSeq}>
