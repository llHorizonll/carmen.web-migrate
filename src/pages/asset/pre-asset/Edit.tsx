...
export default function PreAssetEdit() {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const AssetId = parseInt(id ?? '0', 10);

  const { data: asset, isLoading } = usePreAssetDetail(AssetId);
  const updateMutation = useUpdatePreAsset();
  ...
  return (
    <div>
      <PageHeader
        title={`Edit Pre-Asset: ${assetCode}`}
