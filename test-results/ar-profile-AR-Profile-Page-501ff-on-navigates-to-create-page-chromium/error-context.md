...
- Name: ar/profile.spec.ts >> AR Profile Page >> create button navigates to create page
- Location: tests/e2e/ar/profile.spec.ts:30:3
...
Locator: getByRole('button', { name: /create|new|add/i })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: /create|new|add/i })
...
```ts
  ...
  2   | 
  3   | /**
  4   |  * AR Profile E2E Tests
  5   |  * Tests the Accounts Receivable Profile page functionality
  6   |  */
  7   | 
  8   | test.describe('AR Profile Page', () => {
  9   |   test.beforeEach(async ({ page }) => {
  10  |     // Navigate to AR Profile page
  11  |     await page.goto('/ar/profile');
  12  |   });
  13  | 
  14  |   test('page loads without errors', async ({ page }) => {
  15  |     // Verify page title is visible
  16  |     await expect(page.getByText('AR Profile')).toBeVisible();
  17  |   });
  18  | 
  19  |   test('displays profile data table', async ({ page }) => {
