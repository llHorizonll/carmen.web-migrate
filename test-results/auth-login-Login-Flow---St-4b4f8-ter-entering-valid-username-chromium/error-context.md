...
- Name: auth/login.spec.ts >> Login Flow - Step 1 to Step 2 Transition >> should advance to password step after entering valid username
- Location: tests/e2e/auth/login.spec.ts:97:3
...
Locator: getByLabel('Business Unit (Tenant)')
Expected: visible
Error: strict mode violation: getByLabel('Business Unit (Tenant)') resolved to 2 elements:
    1) <input readonly data-path="tenant" autocomplete="off" aria-invalid="false" data-variant="default" id="mantine-wtganpfmu" aria-haspopup="listbox" value="Development (dev)" placeholder="Select tenant" class="m_8fb7ebe7 mantine-Input-input mantine-Select-input"/> aka getByRole('textbox', { name: 'Business Unit (Tenant)' })
    2) <div role="listbox" id="mantine-5z0s2ogsl" class="m_b2821a6e mantine-Select-options" aria-labelledby="mantine-wtganpfmu-label">…</div> aka locator('div').filter({ hasText: 'SuccessUser validated. Please enter your password.Development (dev)English (' }).getByLabel('Business Unit (Tenant)')

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByLabel('Business Unit (Tenant)')
