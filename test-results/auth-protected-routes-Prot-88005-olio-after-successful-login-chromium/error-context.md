...
- Name: auth/protected-routes.spec.ts >> Protected Routes - Redirect After Login >> should redirect back to AR folio after successful login
- Location: tests/e2e/auth/protected-routes.spec.ts:109:3
...
Call log:
  - Expect "toHaveURL" with timeout 5000ms
    8 × unexpected value "http://localhost:5173/ar/folio"
...
> 114 |     await expect(page).toHaveURL(/\/login/);
  ...
  148 |     
  149 |     // Should be redirected to login
  150 |     await expect(page).toHaveURL(/\/login/);
  151 |     
  152 |     // Login
  153 |     await performLogin(page);
  154 |     
  155 |     // Should redirect back to GL journal voucher
  156 |     await page.waitForURL(/\/gl\/journal-voucher/, { timeout: 10000 });
  157 |     await expect(page).toHaveURL(/\/gl\/journal-voucher/);
  158 |   });
  159 | });
  160 | 
